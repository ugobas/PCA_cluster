...
static int Stationary(float *P_i, float **P_ij, int N);
...
static int Gaussian_parameters(float *tau, float **corr,
			       float **mu, float ***sig,
...
/****************** Main routine ************************/
float Expectation_maximization_order(int *cluster, int ncl,
				     ...
				     int N, int Nvar)
{
 ...
 start_split:
  ...
  for(i=0; i<N; i++){
    w1_ik[i][0]=1; w2_ikk1[i][0][0]=1;
  }
  Gaussian_parameters(tau1,corr1,mu1,sig1,w1_ik,w2_ikk1,x_SamVar,N,Nvar,1,0);
  ...
  // Clusters parameters copied from global statistics
  for(k=1; k<ncl; k++){
    for(j=0; j<Nvar; j++){
      mu1[k][j]=mu1[0][j];
      for(j1=0; j1<=j; j1++){
  ...
  float p=1./(float)ncl;
  float same=0.5, diff=(1-same)/(float)(ncl-1);
  for(k=0; k<ncl; k++){
    tau1[k]=p;
    for(k1=0; k1<ncl; k1++){
      if(k1==k){corr1[k][k1]=same;}
      else{corr1[k][k1]=diff;}
    }
  }
  ...
  float d=sig1[0][j][j]/(float)(l*l), mu=mu1[0][j];
 ...
 Compute_lik:
  ...
  Print_parameters(numcl, tau1, corr1, mu1, sig1, ncl, Nvar, 0);
  Copy_mu(Mu[Round-1], mu1, ncl, Nvar);
  ...
  if(lik_max > lik){
    ...
    Print_parameters(numcl, tau3, corr3, mu3, sig3, ncl, Nvar, 0);
    printf("likelihood= %.0f\n", lik);
  }
  /*if(COMPACT){
  ...
  if(ncl>1){
    double sim=Compare_mu(Mu, Round-1,  ncl, Nvar);
    printf("Similarity between different initial conditions: %.3f\n", sim);
  }
